...
function AppV2({ bookUuid, onBackToShelf }: AppV2Props) {
  ...
  const [currentChapter, setCurrentChapter] = useState(
    initialProgress.current.chapter
  );
  const [targetXpath, setTargetXpath] = useState<string | undefined>(
    initialProgress.current.xpath
  );
  ...
  const [initialShowOriginal, setInitialShowOriginal] = useState<
    boolean | undefined
  ...
  const handleShowOriginalChange = useCallback(
  ...
  const handleProgressChange = useCallback(
    (xpath: string, chapterFraction: number) => {
  ...
  const fetchChapterData = async (
    ...
    const response = await fetchWithRetry(
      `/api/v2/book/${bookUuid}/chapter/${chapterNumber}`
    );
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
  ...
  const prefetchChapter = (chapterNumber: number) => {
    ...
    const run = () => {
      fetchChapterData(chapterNumber).catch(() => {});
    };
  ...
  const handleShare = useCallback(async () => {
    ...
    if (response.ok) {
      ...
      throw new Error(t.book.createShareFailed);
    }
  }, [bookUuid]);
  ...
  const handleRevokeShare = useCallback(async () => {
    ...
    if (response.ok) {
      ...
      throw new Error(t.book.revokeShareFailed);
    }
  }, [bookUuid]);
    ...
    return (
      <div className="App">
        <div style={{ textAlign: 'center', padding: '50px' }}>
          ...
          <button onClick={onBackToShelf}>{t.common.goHome}</button>
        </div>
      </div>
    );
  }
